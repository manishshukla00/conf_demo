...
const tpcMembers = [
  ...
  },
  {
    ...
    affiliation: "Department of Computer Science Gombe State University",
  },
  {
...
const getCountryTag = (affiliation) => {
  if (/\bUSA\b/i.test(affiliation))
    return { label: "USA", color: "bg-blue-100 text-blue-700" };
  if (/India/i.test(affiliation))
    return { label: "India", color: "bg-orange-100 text-orange-700" };
  if (/Nigeria/i.test(affiliation))
    return { label: "Nigeria", color: "bg-green-100 text-green-700" };
  if (/Uzbekistan/i.test(affiliation))
    return { label: "Uzbekistan", color: "bg-teal-100 text-teal-700" };
  if (/Romania/i.test(affiliation))
    return { label: "Romania", color: "bg-red-100 text-red-700" };
  if (/Uganda/i.test(affiliation))
    return { label: "Uganda", color: "bg-yellow-100 text-yellow-700" };
  if (/Algeria/i.test(affiliation))
    return { label: "Algeria", color: "bg-emerald-100 text-emerald-700" };
  if (/Vietnam/i.test(affiliation))
    return { label: "Vietnam", color: "bg-rose-100 text-rose-700" };
  if (/Ghana/i.test(affiliation))
    return { label: "Ghana", color: "bg-amber-100 text-amber-700" };
  if (/Dubai/i.test(affiliation))
    return { label: "UAE", color: "bg-sky-100 text-sky-700" };
  if (/Croatia|Czech/i.test(affiliation))
    return { label: "Europe", color: "bg-purple-100 text-purple-700" };
  return null;
};
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`w-11 h-11 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-white font-bold text-sm flex-shrink-0 shadow-md`}
                  >
                  ...
                  <div className="min-w-0">
                    ...
                    {countryTag && (
                      <span
                        className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full mt-1 ${countryTag.color}`}
                      >
                        {countryTag.label}
                      </span>
                    )}
                ...
                {/* Divider */}
                <div
                  className={`h-px w-full bg-gradient-to-r ${gradient} opacity-30 mb-3`}
                ></div>
